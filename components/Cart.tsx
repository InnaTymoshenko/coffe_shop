...
const Cart = ({ openCartHandler }: Props) => {
	...
	return (
		<div id="main_cart" className="fixed top-0 bottom-0 w-full min-h-screen bg-gray-900/80 z-50 flex justify-end">
			<div
				className="lg:w-[45%] sm:w-full min-h-full bg-gray-900 border-l-2 border-l-gray-700 text-gray-200 flex flex-col gap-8 justify-start p-6 overflow-y-auto"
				ref={cartRef}
			>
				...
				<div className="w-full min-h-[30] flex flex-col justify-start gap-4">
					{cartProducts.map(item => (
						<div
							key={item.id}
							className="w-full min-h-32 rounded-sm border border-gray-400 grid lg:grid-cols-6 sm:grid-cols-1 lg:grid-rows-1 sm:grid-rows-4 content-normal items-center gap-2 overflow-hidden"
						>
							<div className="w-full lg:row-span-1 sm:row-span-2">
								<img
									...
									alt={item.title}
									className="w-full lg:h-32 sm:h-[16rem] object-cover object-center"
								/>
							</div>
							<h3 className="lg:col-span-2 sm:col-span-1 text-left text-xl px-4">{item.title}</h3>
							<div className="lg:col-span-3 sm:col-span-1 py-1 flex flex-col gap-1 items-center justify-center sm:px-4 lg:px-0">
								{item.price.map(pr => (
									<div
										key={`${item.title}-${item.id}-${pr.size}`}
										className="w-full grid grid-cols-5 grid-rows-1 items-center content-normal gap-4"
									>
										{item.category === 'Coffee' && (
										...
										<div className="w-full col-span-2 col-start-2 px-1 flex justify-between items-center border border-gray-800 hover:border-gray-200 rounded-sm transition-all duration-300">
											<span>
												<Minus
													size={14}
